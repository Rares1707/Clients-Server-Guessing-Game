...
int main(int argc, char **argv)
{
    ...
    srand(time(0));
    uint16_t randomNumber = rand() % 100;
    ...
    for(;;) {
        ...
        for(i = 0; i <= fdmax; i++) {
	      ...
	      uint16_t numberReceived;
              if ((nbytes = recv(i, (char*)&numberReceived, sizeof(uint16_t), 0)) <= 0) {
		...
		numberReceived = ntohs(numberReceived);
		printf("%i\n", numberReceived);
		if (numberReceived == randomNumber)
		{
			printf("equal\n");
			nbytes = sprintf(buf,"0");
			send(i,buf, strlen(buf)+1,0);
                 	sendToALL(buf, nbytes);
                 ...
                 else if (numberReceived < randomNumber)
                 {
                 	printf("small\n");
                 	nbytes = sprintf(buf,"-1");
                 	send(i,buf, strlen(buf)+1,0);
                 	sendToALL(buf, nbytes);
                 }
                 else if (numberReceived > randomNumber)
